// GLSL Shader Code (Pulse Fragment)
const pulseFragmentShaderSource = `
...
vec4 effect(vec2 screenSize, vec2 screen_coords) {
    ...
    // Rhythmische Pulseffekte
    float pulse1 = sin(iTime * uSpinSpeed * 0.5) * 0.5 + 0.5;
    float pulse2 = sin(iTime * uSpinSpeed * 0.3 + PI * 0.5) * 0.5 + 0.5;
    float pulse3 = sin(iTime * uSpinSpeed * 0.7 + PI) * 0.5 + 0.5;
    
    // Kombiniere Pulse mit UV-Koordinaten
    float angle = atan(uv.y, uv.x);
    float radius = length(uv);
    
    // Erste Pulse-Welle
    float wave1 = sin(angle * 8.0 + iTime * uSpinSpeed * 0.2) * pulse1;
    // Zweite Pulse-Welle
    float wave2 = cos(angle * 12.0 + iTime * uSpinSpeed * 0.3) * pulse2;
    // Dritte Pulse-Welle
    float wave3 = sin(angle * 16.0 + iTime * uSpinSpeed * 0.4) * pulse3;
    
    // Kombiniere die Wellen
    float combined_wave = (wave1 + wave2 + wave3) * 0.1;
    radius += combined_wave;
    
    // Erstelle neue UV-Koordinaten
...
// Funktion zur Initialisierung des Pulse Shaders
function initPulseShader(gl, vertexShader) {
    const fragmentShader = createShader(gl, gl.FRAGMENT_SHADER, pulseFragmentShaderSource);
    if (fragmentShader) {
        const { program, uniforms } = setupShaderProgram(gl, vertexShader, fragmentShader, 'pulse');

// GLSL Shader Code (WaveGrid Fragment)
const waveGridFragmentShaderSource = `
...
vec4 effect(vec2 screenSize, vec2 screen_coords) {
    ...
    // Kombinierte Grid- und Wellenverformung
    float grid = sin(dot(uv, vec2(12.9898,78.233)) + iTime * uSpinSpeed * 0.3);
    float wave = sin(uv.x * 15.0 + iTime * uSpinSpeed * 0.2) * 0.1;
    wave += cos(uv.y * 15.0 + iTime * uSpinSpeed * 0.2) * 0.1;
    
    uv += 0.03 * vec2(grid, cos(dot(uv, vec2(12.9898,78.233)) + iTime * uSpinSpeed * 0.3));
    uv += vec2(wave, wave);
...
// Funktion zur Initialisierung des WaveGrid Shaders
function initWaveGridShader(gl, vertexShader) {
    const fragmentShader = createShader(gl, gl.FRAGMENT_SHADER, waveGridFragmentShaderSource);
    if (fragmentShader) {
        const { program, uniforms } = setupShaderProgram(gl, vertexShader, fragmentShader, 'wavegrid');
